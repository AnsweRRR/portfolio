...
export default async (req: VercelRequest, res: VercelResponse): Promise<void> => {
  ...
  try {
    ...
    if (!forceRefresh && cachedStatus && (now - cachedStatus.timestamp) < STATUS_CACHE_TTL) {
      console.log('[API] Returning cached status (age:', Math.round((now - cachedStatus.timestamp) / 1000), 'seconds)');
      res.setHeader('X-Cache', 'HIT');
      res.setHeader('X-Cache-Age', Math.round((now - cachedStatus.timestamp) / 1000).toString());
      res.status(200).json(cachedStatus.data);
      return;
    }

    console.log('[API]', forceRefresh ? 'Force refresh requested' : 'Cache miss', '- fetching from Tuya API');
    ...
    console.log('[API] Cached new status data (TTL:', STATUS_CACHE_TTL / 1000, 'seconds)');
